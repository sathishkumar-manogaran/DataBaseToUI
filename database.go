...
func checkDatabase() {
	...
	for !databaseCheck {
		...
		if err != nil {
			...
			sqlDB, _ := database.DB()
			if err != nil {
				fmt.Println("Opening main database also:: " + err.Error())
			...
			if !productionDatabase.Migrator().HasTable(&Data{}) {
				...
				if err != nil {
					fmt.Println("Cannot create Data table:: " + err.Error())
					return
				}
			} else {
				fmt.Println("Updating Data Table")
				err := productionDatabase.Migrator().AutoMigrate(&Data{})
				if err != nil {
					fmt.Println("Cannot update Data table")
					return
				}
			}
	...
	fmt.Println("Checking Database is done")
}
